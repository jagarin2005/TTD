...
<?php 
  ...
  $min_date = date('Y-m-d');
  $stmt2 = $conn->prepare("SELECT staff.staff_id, shifts.s_date FROM shifts, staff WHERE staff.staff_id = shifts.staff_id AND shifts.staff_id = :id AND shifts.s_date > :min_date");
  $stmt2->execute(array(":id"=>$q,":min_date"=>$min_date));
